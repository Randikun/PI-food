...
export function validate(state){
    ...
    }else if(!state.healthiness || state.healthiness > 100 || state.healthiness < 0){
        ...
        <div className={`${s.back}`}>
            ...
            <form onSubmit={e=>handleSubmit(e)}>
                <div className={`${s.caja}`}>
                    <label>Title:</label>
                    <input  type='text' name='title' value={state.title} placeholder='Title' required onChange={e=>handleInputChange(e)}/>
                </div>
                    {errors.title && (<h5 className='error'>{errors.title}</h5>)}
                <div className={`${s.caja}`}>
                    <label>Summary:</label>
                    <input type='text' name='summary' value={state.summary} placeholder='Summary' required onChange={e=>handleInputChange(e)}/>

                </div>
                    {errors.summary && (<h5 className='error'>{errors.summary}</h5>)}
                <div className={`${s.caja}`}>
                    <label>Score:</label>
                    <input type='number' name='score' value={state.score} placeholder='Score' onChange={e=>handleInputChange(e)}/>

                </div>
                    {errors.score && (<h5 className='error'>{errors.score}</h5>)}
                <div className={`${s.caja}`}>
                    <label>Healthiness:</label>
                    <input type='text' name='healthiness' value={state.healthiness} placeholder='Healthiness' onChange={e=>handleInputChange(e)}/>

                </div>
                    {errors.healthiness && (<h5 className='error'>{errors.healthiness}</h5>)}
                <div className={`${s.caja}`}>
                    <label>Image:</label>
                    <input type='url' name='image' value={state.image} placeholder='image url' onChange={e=>handleInputChange(e)}/>

                </div>
                    {errors.image && (<h5 className='error'>{errors.image}</h5>)}
                <div className={`${s.caja}`}>
                    <label>Step by step:</label>
                    <input type='text' name='steps' value={state.steps} placeholder='steps' onChange={e=>handleInputChange(e)}/>
                ...
                <div className={`${s.caja}`}>
                    ...
                    {dietas.map(diet=>{return(
                         <span><input key={`${diet.id}`} type='checkbox' value={`${diet.name}`} name={`${diet.name}`} onChange={e=>handleCheck(e)}/>{diet.name}</span>
                    )})}
                ...
                {state.title && state.summary? <button type='submit'>CREATE</button>
                : null }
